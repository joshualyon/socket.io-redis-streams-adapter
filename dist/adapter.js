...
class RedisStreamsAdapter extends socket_io_adapter_1.ClusterAdapterWithHeartbeat {
    ...
    async restoreSession(pid, offset) {
        ...
        // during recovery.
        const maxXrangeCalls = Math.ceil(this.#opts.maxLen / this.#opts.readCount) * 2;
        for (let i = 0; i < maxXrangeCalls; i++) {
            const entries = await (0, util_1.XRANGE)(this.#redisClient, this.#opts.streamName, RedisStreamsAdapter.nextOffset(offset), "+", this.#opts.readCount);
            if (entries.length === 0) {
                break;
            }
